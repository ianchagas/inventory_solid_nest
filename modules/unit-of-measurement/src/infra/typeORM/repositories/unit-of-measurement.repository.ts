...
import { QueryUnDTO } from 'modules/unit-of-measurement/src/dto/request/query-un.dto';
import { UnitOfMeasurementDTO } from 'modules/unit-of-measurement/src/dto/request/unit-of-measurement.dto';
...
class UnitOfMeasurementRepository implements IUnitOfMeasurementRepository {
  ...
  async findById(id: number): Promise<UnitOfMeasurementEntity> {
    const FindUnById = this.unitOfMeasurementRepository.findOne({
      where: {
        id,
      },
    });
